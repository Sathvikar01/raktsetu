...
/**
 * Demo-journey rows (externalDonationId prefix "demo-", generated by the
 * public how-it-works demo) are synthetic: they must never move public
 * aggregates, no matter how often the demo is replayed.
 */
const NOT_DEMO_DONATION = {
  NOT: { externalDonationId: { startsWith: "demo-" } },
} as const;
...
async function queryCommunityStats(min: number): Promise<CommunityStats> {
  const [donations, components, transfusions, bbCount, hospCount, byType] = await Promise.all([
    prisma.donation.count({ where: NOT_DEMO_DONATION }),
    prisma.bloodComponent.count({ where: { donation: NOT_DEMO_DONATION } }),
    prisma.lifecycleEvent.count({
      where: {
        eventType: "COMPONENT_TRANSFUSED",
        verificationStatus: "VERIFIED",
        supersededByCorrection: false,
        component: { donation: NOT_DEMO_DONATION },
      },
    }),
