...
import {
  DEMO_BLOOD_BANK_NAME,
  DEMO_HOSPITAL_NAME,
  simulateComponents,
  simulateFullJourney,
  simulateProcessing,
  simulateReceiveAtHospital,
  simulateRecordDonation,
  simulateTransferToHospital,
  simulateTransfusion,
...
// ---------------------------------------------------------------------------
// Simulator (gated by can(role,"simulator:use") AND process.env.DEMO_MODE==="true")
// ---------------------------------------------------------------------------

async function assertSimulatorAccess(): Promise<void> {
  const user = await requireRole("ORG_STAFF", "ORG_ADMIN");
  if (!can(user.role, "simulator:use")) throw new ForbiddenError();
  if (process.env.DEMO_MODE !== "true") throw new ForbiddenError();
  const demoOrgs = await prisma.organization.findMany({
    where: { name: { in: [DEMO_BLOOD_BANK_NAME, DEMO_HOSPITAL_NAME] } },
    select: { id: true },
  });
  for (const org of demoOrgs) {
    if (await gateOrgMembership(org.id)) return;
  }
  throw new ForbiddenError();
}

export async function simulateRecordDonationStep(donorEmail: string | null): Promise<OpsActionState> {
  try {
    await assertSimulatorAccess();
  } catch (err) {
    return opsFailure(err);
  }
  try {
    const result = await simulateRecordDonation({ donorEmail: donorEmail?.trim() || undefined });
    return {
      ok: true,
      message: result.label,
      step: "record",
      linkCode: result.linkCode,
      simDonationId: result.donationId,
    };
  } catch (err) {
    return opsFailure(err);
  }
...
export async function simulateProcessingStep(donationId: string | null): Promise<OpsActionState> {
  try {
    await assertSimulatorAccess();
  } catch (err) {
    return opsFailure(err);
  }
  if (!donationId) return { ok: false, message: getDictionary().staff.simNeedsDonation };
  try {
    const result = await simulateProcessing(donationId);
    return { ok: true, message: result.label, step: "processing", simDonationId: donationId };
  } catch (err) {
    return opsFailure(err);
  }
}

export async function simulateComponentsStep(donationId: string | null): Promise<OpsActionState> {
  try {
    await assertSimulatorAccess();
  } catch (err) {
    return opsFailure(err);
  ...
  if (!donationId) return { ok: false, message: getDictionary().staff.simNeedsDonation };
  try {
    const result = await simulateComponents(donationId);
    return {
      ok: true,
      message: result.label,
      step: "components",
      simDonationId: donationId,
      simRbcComponentId: result.componentIds.RBC,
    };
  } catch (err) {
    return opsFailure(err);
  }
...
export async function simulateTransferStep(rbcComponentId: string | null): Promise<OpsActionState> {
  try {
    await assertSimulatorAccess();
  } catch (err) {
    return opsFailure(err);
  }
  if (!rbcComponentId) return { ok: false, message: getDictionary().staff.simNeedsComponent };
  try {
    const result = await simulateTransferToHospital(rbcComponentId);
    return {
      ok: true,
      message: result.label,
      step: "transfer",
      simRbcComponentId: rbcComponentId,
    };
  } catch (err) {
    return opsFailure(err);
  }
...
export async function simulateReceiveStep(rbcComponentId: string | null): Promise<OpsActionState> {
  try {
    await assertSimulatorAccess();
  } catch (err) {
    return opsFailure(err);
  ...
  if (!rbcComponentId) return { ok: false, message: getDictionary().staff.simNeedsComponent };
  try {
    const result = await simulateReceiveAtHospital(rbcComponentId);
    return { ok: true, message: result.label, step: "receive", simRbcComponentId: rbcComponentId };
  } catch (err) {
    return opsFailure(err);
  }
}

export async function simulateTransfusionStep(rbcComponentId: string | null): Promise<OpsActionState> {
  try {
    await assertSimulatorAccess();
  } catch (err) {
    return opsFailure(err);
  ...
  if (!rbcComponentId) return { ok: false, message: getDictionary().staff.simNeedsComponent };
  try {
    const result = await simulateTransfusion(rbcComponentId);
    return { ok: true, message: result.label, step: "transfusion", simRbcComponentId: rbcComponentId };
  } catch (err) {
    return opsFailure(err);
  }
}

export async function simulateFullJourneyStep(donorEmail: string | null): Promise<OpsActionState> {
  try {
    await assertSimulatorAccess();
  } catch (err) {
    return opsFailure(err);
  ...
  try {
    const result = await simulateFullJourney({ donorEmail: donorEmail?.trim() || undefined });
    return {
      ok: true,
      message: result.banner,
      step: "full",
      linkCode: result.donation.linkCode,
      simDonationId: result.donation.id,
      simRbcComponentId: result.components.RBC,
