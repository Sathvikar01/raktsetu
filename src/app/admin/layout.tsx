...
import { CSRF_COOKIE } from "@/lib/auth/csrf-cookie";
import { Badge } from "@/packages/ui";
import { CsrfProvider } from "@/components/site/Csrf";
import { CsrfInput } from "@/components/site/CsrfInput";
import { signOutAdminAction } from "./actions";
import { PortalTabs } from "../staff/components/PortalTabs";
        ...
        <header className="sticky top-0 z-40 border-b border-ink/10 bg-white/90 backdrop-blur">
          <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
            ...
            <div className="flex items-center gap-3">
              <PortalTabs
                showAdmin
                showPlatform={user.role === "PLATFORM_ADMIN"}
              />
              <span className="hidden text-sm text-ink-soft sm:inline">
                {user.displayName}
                <Badge tone="teal" className="ml-2">
                  {user.role === "PLATFORM_ADMIN" ? user.role : d.staff.roleAdmin}
                </Badge>
              </span>
